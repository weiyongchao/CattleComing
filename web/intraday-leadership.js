  ...
  const enhance = async () => {
    ...
    try {
      ...
      cards.forEach((card, index) => {
        ...
        evidence.style.cssText = `color:${candidate.limit_down_reversal ? "var(--red)" : candidate.card_anomaly ? "var(--amber)" : "#bfd0ca"};font-size:12px;line-height:1.8;margin:4px 0 8px`;
        evidence.textContent = candidate.card_anomaly
          ? `${candidate.signal} · 10日涨幅代理${candidate.regulatory_risk?.ten_day_change?.toFixed(2) ?? "--"}% · ${candidate.entry_action} · 建议仓位上限${candidate.position_cap_percent}%`
          : candidate.limit_down_reversal
        ...
        if ((candidate.limit_down_reversal || candidate.card_anomaly) && candidate.risks?.length) {
      ...
      if (count) count.textContent = `展示 ${data.candidates.length} 只 · 卡异动 ${data.card_anomaly_count || 0} 只 · 高风险反核 ${data.reversal_count || 0} 只 · 合格 ${data.qualified_count} 只`;
      const note = document.getElementById("intradayNote");
      if (note) {
        note.textContent = `技术合格 ${data.technical_qualified_count} 只 · 龙头成熟度淘汰 ${data.leadership_filtered_count} 只 · 卡异动 ${data.card_anomaly_count || 0} 只（尾盘可执行${data.tail_buy_ready_count || 0}只） · 高风险反核 ${data.reversal_count || 0} 只 · 最终合格 ${data.qualified_count} 只。${data.method} ${data.disclaimer}`;
      }
    } catch (_) {
